...
sinkhorn_log <- function(cost_mat, a, b,
                         eps = 100, max_iter = 500, tol = 1e-6) {
  NT <- length(a); NC <- length(b)
  log_a <- log(a); log_b <- log(b)
  M     <- -cost_mat / eps
  log_u <- rep(0, NT); log_v <- rep(0, NC)
...
sk2 <- function(y1, y0, gf, mo, eps) {
  NT <- length(y1); NC <- length(y0)
  a  <- rep(1/NT, NT); b <- rep(1/NC, NC)
  g_mat <- outer(y1, y0, Vectorize(gf))
  if (mo == "max") return(-sinkhorn_log(-g_mat, a, b, eps))
  sinkhorn_log(g_mat, a, b, eps)
}
...
# ── バウンド計算 ──────────────────────────────────────────────────────────────
compute_bounds <- function(y1, y0, solver = c("lp","sinkhorn"), eps = 100) {
  solver <- match.arg(solver)
  acde <- mean(y1) - mean(y0)
  lp2  <- function(gf, mo) {
    if (solver == "lp") cot_lp(y1, y0, outer(y1, y0, Vectorize(gf)), mo)
    else sk2(y1, y0, gf, mo, eps)
  }
...
run_bootstrap <- function(y1, y0, B = 200, label = "",
  ...
  boot_list <- lapply(seq_len(B), function(b) {
    ...
    res <- compute_bounds(y1b, y0b, solver=solver, eps=eps)
...
run_bootstrap(y1_la_r, y0_la_r, B=100, label="Y1 - Y0  /  Y1/Y0",
              solver="sinkhorn", eps=1000000000)
